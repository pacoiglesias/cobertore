    ...
    >
      ...
      <div
        className={`absolute inset-0 ${
          isTextil
            ? "bg-gradient-to-t from-[#0a0f1d] via-[#0a0f1d]/85 to-[#0a0f1d]/60"
            : "bg-gradient-to-t from-[#070b14] via-[#070b14]/85 to-[#070b14]/60"
        }`}
      />
      
      {/* Icon */}
      <div className="absolute top-10 right-10 z-20">
         <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border backdrop-blur-md shadow-2xl transition-colors ${
            isTextil ? "bg-white/5 border-white/10 text-white group-hover:border-white/30" : "bg-amber-600/10 border-amber-500/20 text-amber-500 group-hover:border-amber-500/50"
         }`}>
      ...
      <div className="relative flex flex-col h-full z-20">
        <SectionEyebrow className={isTextil ? "text-slate-400" : "text-amber-500"}>{eyebrow}</SectionEyebrow>
        <h3 className="font-serif text-white text-3xl mt-6 leading-snug drop-shadow-md">{title}</h3>
        <p className="text-slate-400 mt-5 leading-relaxed font-light">{copy}</p>
        <ul className="mt-8 space-y-3 font-mono text-[12px] tracking-wide text-slate-300">
          {bullets.map((b: string) => (
            <li key={b} className="flex items-start gap-3">
              <ChevronRight size={16} className={`${isTextil ? "text-slate-500" : "text-amber-500"} mt-0.5 shrink-0`} />
              {b}
            </li>
          ))}
        </ul>
